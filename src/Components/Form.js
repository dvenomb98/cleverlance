...
const validate = (values) => {
  ...
  const alphaRegex = /^([ěščřžýáíéóúůďťňĎŇŤŠČŘŽÝÁÍÉÚŮĚÓa-zA-z]){2,20}$/
  ...
  if (!values.firstName) {
    errors.firstName = "Jméno je vyžadováno!"
      ...
      errors.firstName = "Nedosažen minimální počet znaků!"
    }
    else if (values.firstName.length > 20) {
      errors.firstName = "Překročen maximální počet znaků!"
    }
  ...
  if (!values.lastName) {
    ...
    errors.lastName ="Příjmení obsahuje nepovolené znaky!"
  }

  if (!values.rodneCislo) {
    errors.rodneCislo = "Rodné číslo je vyžadováno!"
  }
  ...
  if (!values.email) {
    ...
    errors.email ="Emailová adresa není správná!"
  }

  if (!values.number) {
    errors.number = "Telefon je vyžadován!"
  }
  else if (values.number.length !== 9)
  { errors.number = "Nesprávné telefonní číslo!"}
  ...
  return (
    ...
    <div className="w-2/3 xl:w-1/2 mx-auto flex flex-col items-start justify-center gap-10">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col w-full text-lg ">
          ...
          <div className="flex flex-col gap-1 lg:flex-row lg:justify-between mt-5 lg:items-center">
            <label htmlFor="rc">Rodné číslo:</label>
